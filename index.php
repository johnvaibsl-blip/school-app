...
if (file_exists($landing)) {
    ...
    if (isLoggedIn()) {
        ...
        $roleData = '<script>window.__USER_ROLE="' . htmlspecialchars($_SESSION['role']) . '";window.__USER_ID=' . intval($_SESSION['user_id'] ?? 0) . ';window.__USER_NAME="' . htmlspecialchars($_SESSION['user_name'] ?? '') . '";window.__USER_CLASS="' . htmlspecialchars($_SESSION['user_class'] ?? 'Class 8') . '";window.__USER_EMAIL="' . htmlspecialchars($_SESSION['user_email'] ?? '') . '";</script>';
